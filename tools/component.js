...
function write () {
	mkdirp(dir, err => {
		...
		Promise.all([
			type === 'section' ?
			template(path.resolve(__dirname, 'template/Section.js'), path.resolve(dir, `${name}.js`)) :
			template(path.resolve(__dirname, 'template/Component.js'), path.resolve(dir, `${name}.js`)),
			template(path.resolve(__dirname, 'template/Component.css'), path.resolve(dir, `${name}.css`)),
			template(path.resolve(__dirname, 'template/index.js'), path.resolve(dir, `index.js`)),
		]).then(() => {
